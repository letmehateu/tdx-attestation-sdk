...
#[cfg(target_arch = "x86_64")]
pub fn get_hypervisor() -> Result<Hypervisor> {
    ...
    if (cpuid.ecx & CPUID_FEATURE_HYPERVISOR) == 0 {
        return Err(CocoError::Firmware(
            "Not running under a hypervisor!".to_string(),
        ));
    }
    ...
    Err(CocoError::Firmware("Unknown hypervisor!".to_string()))
}
...
pub fn get_hypervisor() {
    // TODO: arm does not support CPUID like x86_64 does.
}
